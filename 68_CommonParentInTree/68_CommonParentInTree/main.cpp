...
bool GetPathToNode(const TreeNode* pRoot, const TreeNode* pNode, std::list<const TreeNode*>& path) {
    if (pRoot == pNode) {
        return true;
    }
    ...
    bool found = false;
    auto it = pRoot->m_vChildren.begin();
    for (; it != pRoot->m_vChildren.end(); it++) {
        found = GetPathToNode(*it, pNode, path);
        if (found)
            break;
    }
...
const TreeNode* GetCommonNode(std::list<const TreeNode*> l1, std::list<const TreeNode*> l2) {
...
const TreeNode* GetLastCommonParent(const TreeNode* pRoot, const TreeNode* pNode1, const TreeNode* pNode2) {
    ...
    std::list<const TreeNode*> pathToNode1;
    GetPathToNode(pRoot, pNode1, pathToNode1);
    
    std::list<const TreeNode*> pathToNode2;
    GetPathToNode(pRoot, pNode2, pathToNode2);
    
    return GetCommonNode(pathToNode1, pathToNode2);
}
