...
void Print(BinaryTreeNode* pRoot) {
    ...
    while (!st[0].empty() || !st[1].empty()) {
        BinaryTreeNode* pNode = st[currStack].top();
        st[currStack].pop();
        ...
        if (currStack == 0) {
            if (pNode->m_pLeft)
                st[nextStack].push(pNode->m_pLeft);
            if (pNode->m_pRight)
                st[nextStack].push(pNode->m_pRight);
        }
        else {
            if (pNode->m_pRight)
                st[nextStack].push(pNode->m_pRight);
        ...
        if (st[currStack].empty()) {
            printf("\n");
            currStack = 1 - currStack;
            nextStack = 1 - nextStack;
        }
    }
}
