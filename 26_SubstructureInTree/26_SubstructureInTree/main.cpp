...
/*
 思路：求A树是否包含B树（🌲）
 1. 既然要知道是否是子树，我们就要先找到一个共同的根节点，这个可以用先序遍历的方式实现。
 2. 当我们找到根节点相同的，我们要判断一下，一棵树是否包含另一棵树。
 */
...
bool doesTree1HasTree2(BinaryTreeNode* pRoot1, BinaryTreeNode* pRoot2) {
    if (pRoot2 == nullptr) return true;     // 表示不需要匹配，叶子节点相同，两个子树又都为空
    if (pRoot1 == nullptr) return false;    // pRoot2还需要匹配的时候，pRoot1就空了，表示匹配失败
