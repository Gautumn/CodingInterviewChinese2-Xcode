...
void DeleteDuplication(ListNode** pHead) {
    ...
    while (pNode != nullptr) {
        ...
        if (pNext != nullptr && pNext->m_nValue == pNode->m_nValue) // 注意判空
            deleteFlag = true;
        ...
        if (!deleteFlag) {
            ...
            // 存在重复的，pNode指向的就是第一个需要删除的节点。
            int value = pNode->m_nValue;
            ListNode* pToBeDelete = pNode;
            // 注意判空，循环结束后，pNext就是那个不是重复的节点。
            ...
            if (pPreNode == nullptr)
                *pHead = pNext;
                ...
                pPreNode->m_pNext = pNext;  // 中间重复的被删除了，需要连接前一个节点和后一个节点
