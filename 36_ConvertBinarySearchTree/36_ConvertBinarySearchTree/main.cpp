...
void ConvertCore(BinaryTreeNode* pRootOfTree, BinaryTreeNode** pLastNodeInList);
...
// 错误：涉及到头指针的修改，要用指针的指针
void ConvertCore(BinaryTreeNode* pRootOfTree, BinaryTreeNode** pLastNodeInList) {
    if (pRootOfTree == nullptr) return;
    
    if (pRootOfTree->m_pLeft)
        ConvertCore(pRootOfTree->m_pLeft, pLastNodeInList);
    ...
    *pLastNodeInList = pRootOfTree;
    ...
    if (pRootOfTree->m_pRight)
        ConvertCore(pRootOfTree->m_pRight, pLastNodeInList);
}
