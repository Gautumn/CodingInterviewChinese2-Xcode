...
bool isSymmetrical(BinaryTreeNode* pRootLeft, BinaryTreeNode* pRootRight);
...
bool isSymmetrical(BinaryTreeNode* pRootLeft, BinaryTreeNode* pRootRight) {
    if (pRootLeft == nullptr && pRootRight == nullptr)
        return true;
    
    if (pRootLeft == nullptr || pRootRight == nullptr)
        return false;
    
    if (pRootLeft->m_nValue != pRootRight->m_nValue)
        return false;
    
    return isSymmetrical(pRootLeft->m_pLeft, pRootRight->m_pRight) &&
           isSymmetrical(pRootLeft->m_pRight, pRootRight->m_pLeft);
}
...
/*
 ...
 思路：用两个前序遍历，一个先向左走，一个向右走，如果二者相同我们认为树是对称的
 前序向左：8，6，5，7，6，7，5
 前序向右：8，6，5，7，6，7，5
 */
