...
// data in [start, end)
int lower_bound(const int* data, int start, int end, int k) {
    while (start < end) {
...
int upper_bound(const int* data, int start, int end, int k) {
    while (start < end) {
        int mid = start + (end - start) / 2;
        if (k >= data[mid]) start = mid + 1;
        else end = mid;
...
int GetNumberOfK(const int* data, int length, int k) {
    int lower = lower_bound(data, 0, length, k);
    if (lower == length || data[lower] != k)
        return 0; // not found
    int upper = upper_bound(data, 0, length, k);
    return upper - lower;
}
